...
<table>
    <tr>
        <td>
    <div class="card">
    <div class="card-img"><img src="../../img/impresorabn.png" width="150px" height="150px"></div>
        <div class="card-title">Impresiones a B/N</div>
        <div class="card-subtitle">$ 2.00 c/u </div>
        <hr class="card-divider">
        <div class="card-footer">
        <div class="card-price">
        <form action="../pedidos.php" method="POST">
            <label style="font-size: 15px; font-weight: bold;">No.</label>
            <input required="" class="input" type="number" name="NuImpBn" id="NIB" min="1" max="200" style="width: 50px; height: 25px;">
        </div>
        <div class="card-price" id="Totalb"></div>
        <input type="hidden" name="Tipo_serv" value="Blanco y negro">
        <input type="hidden" name="valor_pre" value="impresiones">
        <input type="hidden" name="totalValue" id="totalValorb">
        <script>
            const multb = 2;
            const inputnumb = document.getElementById('NIB');
            const resultadob = document.getElementById('Totalb');
            const totalValorbInput = document.getElementById('totalValorb');
            inputnumb.addEventListener('input', function(){
                const valorb = inputnumb.value;
                const Totalb = valorb*multb;
                resultadob.textContent = `$ ${Totalb}`;
                totalValorbInput.value = Totalb;
            });
        </script>
        ...
        <td>
        <div class="card">
        <div class="card-img"><img src="../../img/impresora.png" width="120px" height="120px" id="imgcolor"></div>
        <div class="card-title">Impresiones a color </div>
        <div class="card-subtitle">$ 5.00 c/u </div>
        <hr class="card-divider">
        <div class="card-footer">
        <div class="card-price">
        <form action="../pedidos.php" method="POST">
            <label style="font-size: 15px; font-weight: bold;">No.</label>
            <input required="" class="input" type="number" name="NuImpCol" id="NIC" min="1" max="200" style="width: 40px; height: 25px;">
        </div>
        <div class="card-price" id="Total"> </div>
        <input type="hidden" name="Tipo_serv" value="Color">
        <input type="hidden" name="valor_pre" value="impresiones">
        <input type="hidden" name="totalValue" id="totalValue">
    <script>
        const mult = 5;
        const inputnum = document.getElementById('NIC');
